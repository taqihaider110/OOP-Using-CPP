...
//Assuming ANY node means any of the 4 types (head,tail,position,value)
		...
		void sortlist(){		//sorting by data , bubble sort
			node* temp=head;
			while(temp->nextnode!=NULL) {
				node* temp2=head;
...
int main(){
	DLL l,m;
	for(int i=2;i<11;i+=2){  //Initializing L and M
		l.insertAttail(i);
		m.insertAttail(i-1);	
	}
	...
	//Question 3 part starts here
	
	cout<<"-----------LIST N after Desc Sort-------------------------"<<endl;
	n.sortlist();		//calls sorting function
	n.display();
