...
class CLL{
	...
	int nodecount=0;
	public:
	void setlist(node* h,int n){
		head=h;
		nodecount=n;
	}
	void makehalves(CLL &obj){
		int middleposition = (int)nodecount/2;     //Amount for 1st half
		int secondhalf = nodecount-middleposition;  //Amount for 2nd half
		int count=0;
		node* temp=head;
		node* prev=temp;
		while(temp->next!=head&&count<middleposition) {  // prev is last element of first half, temp is head of second half
			prev=temp;
			temp=temp->next;
			count++;
		}
		prev->next=head; //detach prev from remaining half, circular link back to start
		node* secondhead=temp;
		while(temp->next!=head){
			temp=temp->next;
		}
		temp->next=secondhead;
		obj.setlist(secondhead,secondhalf);
	}
...
int main(){
	...
	while(input!=0.5) {
		...
		cout<<"1. Insert\t2. Delete\t3. Update\t4. Halve the Linked List [Halving will end the program]\nEnter 0.5 to Exit\n[Anything else will default to Delete]\n";
		cin>>input;
		if(input==1) {
			...
			cout<<"Halving list"<<endl;
			list.makehalves(list2);
			cout<<"First Half list---------------------------------"<<endl;
			list.print();
			cout<<"Second Half list--------------------------------"<<endl;
			list2.print();
			return 0;
		}
		else{
